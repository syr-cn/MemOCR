...
from recurrent.impls.call_html_renderer import batch_generate_images, batch_process_images
...
You should draft the memory in HTML format. In your HTML draft, you may use different font sizes, styles and layouts (e.g., bolding, coloring, italicizing, etc.) to arrange the information.
Write memory based following human intuition:
- more important information should be emphasized and more visible (larger font size, bolder, colored red, etc.), in case the rendered image can be clearly read.
- use proper spacing and layout to make the HTML draft easy to read (listing, numbering, etc.)
No need to wrap your HTML draft with ```html or ```, just write the HTML draft directly.
The draft memory, in HTML format:
"""
    ...
    def action(self) -> Tuple[List[list], dict]: # <-- Changed return type
        ...
        if active_mask.sum().item() == 0:
            ...
            for i in range(self.bsz):
                prompt_text = self.tokenizer.decode(prompts[i][:self.config.max_prompt_length], skip_special_tokens=True)
                memory_tokens = self.memory[i][:self.config.max_memorization_length] if self.memory[i] is not None else self.NO_MEMORY_TOKENS
            ...
            logger.info(f'start generating {len(all_memory_text)} memory images')
            all_memory_images = batch_generate_images(all_memory_text, max_workers=num_workers)
            logger.info(f'finished generating {len(all_memory_text)} memory images. Time taken: {time.time() - start_time} seconds')

            start_time = time.time()
            logger.info(f'start processing {len(all_memory_images)} generated memory images')
            all_memory_images = batch_process_images(all_memory_images, max_workers=num_workers)
            none_num = sum(1 for img in all_memory_images if img is None)
            logger.info(f'finished processing {len(all_memory_images)} generated memory images. {none_num} images are None. Time taken: {time.time() - start_time} seconds')
