...
import os
MemoryBudget = int(os.getenv("MAX_PIXELS_28_28", 1024))
    ...
    async with session:
        ...
        for i in range(0, len(input_ids), RECURRENT_CHUNK_SIZE):
            ...
            try:
                ...
                traceback.print_exc()
                return '', {}
        memory_tokens = tokenizer.encode(memory)
        truncated_memory_tokens = memory_tokens[:MemoryBudget] # truncate the memory to the memory budget
        truncated_memory = tokenizer.decode(truncated_memory_tokens) 

        msg = TEMPLATE_FINAL.format(prompt=prompt, memory=truncated_memory)
        if idx == 0:
            print("user:")
            print(clip_long_string(msg))
        try:
