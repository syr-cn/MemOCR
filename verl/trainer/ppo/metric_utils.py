...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> dict[str, Any]:
    ...
    if 'token_level_scores_vanilla_qa' in batch.batch:
        assert 'token_level_scores_subsampled_qa' in batch.batch and 'token_level_scores_gap_fill' in batch.batch
        sequence_score_dict = {
            "vanilla_qa": batch.batch["token_level_scores_vanilla_qa"].sum(-1),
            "subsampled_qa": batch.batch["token_level_scores_subsampled_qa"].sum(-1),
            "gap_fill": batch.batch["token_level_scores_gap_fill"].sum(-1)
        }
        sequence_score_metrics = {
            f"critic/score/{key}/mean": torch.mean(sequence_score_dict[key]).detach().item()
            for key in sequence_score_dict.keys()
        }
    else:
        sequence_score_metrics = {}

    advantages = batch.batch["advantages"]
    if 'advantages_vanilla_qa' in batch.batch:
        assert 'advantages_subsampled_qa' in batch.batch and 'advantages_gap_fill' in batch.batch
        advantages_dict = {
            "vanilla_qa": batch.batch["advantages_vanilla_qa"],
            "subsampled_qa": batch.batch["advantages_subsampled_qa"],
            "gap_fill": batch.batch["advantages_gap_fill"]
        }
        advantages_metrics = {
            f"critic/advantages/{key}/mean": torch.mean(advantages_dict[key]).detach().item()
