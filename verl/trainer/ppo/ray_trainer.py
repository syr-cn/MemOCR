...
class RayPPOTrainer:
    ...
    def __init__(
        ...
        if 'vl' in config.actor_rollout_ref.model.path.lower():
            assert processor is not None, "Processor is required for Qwen-VL model"
        ...
        self._register_recurrent()
        self._create_dataloader(train_dataset, val_dataset, collate_fn, train_sampler)

    def _register_recurrent(self):
        config = self.config
    ...
    def _create_dataloader(self, train_dataset, val_dataset, collate_fn, train_sampler: Optional[Sampler]):
        ...
        if self.config.recurrent.enable:
            train_dataset = self.recurrent_register.dataset_cls(
        ...
        if self.config.recurrent.enable:
            val_dataset = self.recurrent_register.dataset_cls(
                recurrent_config=self.recurrent_config,
                data_config=self.config.data,
    ...
    def _get_gen_batch(self, batch: DataProto, dataset=None) -> DataProto:
        reward_model_keys = set({"data_source", "reward_model", "extra_info", "uid"}) & batch.non_tensor_batch.keys()

        # pop those keys for generation

        if self.config.recurrent.enable:
            batch_keys_to_pop, non_tensor_batch_keys_to_pop = dataset.get_bactch_keys()
        else:
    ...
    def _validate(self):
        ...
        for test_data in self.val_dataloader:
            ...
            test_gen_batch = self._get_gen_batch(test_batch, self.val_dataset)
            ...
            if not self.config.recurrent.enable:
                ...
                os.environ['GLOBAL_PERTURB_SWITCH'] = "0" # turn off perturbation for validation
                print(f"GLOBAL_PERTURB_SWITCH before validation: {os.environ['GLOBAL_PERTURB_SWITCH']}")
                from recurrent.utils import final_batch
                if 'vl' in self.config.trainer.experiment_name.lower():
                    assert 'vl' in self.config.actor_rollout_ref.model.path.lower(), "VL models are required for VL experiments"
                    output_gen_batch, final_mask, sample_index = self.generation_manager.run_llm_loop_vl(test_gen_batch, {})
                else:
                ...
                os.environ['GLOBAL_PERTURB_SWITCH'] = "1" # turn on perturbation for training
                print(f"GLOBAL_PERTURB_SWITCH after validation: {os.environ['GLOBAL_PERTURB_SWITCH']}")
    ...
    def init_workers(self):
        ...
        if self.config.recurrent.enable:
            if not self.async_rollout_mode:
                ...
                raise NotImplementedError("Async mode is not supported for Qwen-VL model")
    ...
    def fit(self):
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                gen_batch = self._get_gen_batch(batch, self.train_dataset)
                ...
                with marked_timer("step", timing_raw):
                    ...
                    with marked_timer("gen", timing_raw, color="red"):
                        ...
                        # with _timer('step', timing_raw):
                            ...
                            enable_triple_turn = 'triple' in self.config.trainer.experiment_name.lower()
                            print(f'Triple Turn Status: enable_triple_turn={enable_triple_turn}')
                            if enable_triple_turn:
                                assert 'vl' in self.config.actor_rollout_ref.model.path.lower(), "Triple Turn only support VL models for now"
                                gen_batch_output, (final_mask, vanilla_qa_mask, subsampled_qa_mask, gap_fill_mask), sample_index, all_gap_fill_answers = self.generation_manager.run_llm_loop_vl_triple_turn(gen_batch, timing_raw)

                                assert final_mask.sum().item() == len(batch.batch) * 3, \
                                    f"The number of final responses should be equal to the number of prompts. Got: {final_mask.sum().item()} != {len(batch.batch) * 3}"
                                assert vanilla_qa_mask.sum().item() == len(batch.batch), \
                                    f"The number of vanilla QA responses should be equal to the number of prompts. Got: {vanilla_qa_mask.sum().item()} != {len(batch.batch)}"
                                assert subsampled_qa_mask.sum().item() == len(batch.batch), \
                                    f"The number of subsampled QA responses should be equal to the number of prompts. Got: {subsampled_qa_mask.sum().item()} != {len(batch.batch)}"
                                assert gap_fill_mask.sum().item() == len(batch.batch), \
                                    f"The number of gap fill responses should be equal to the number of prompts. Got: {gap_fill_mask.sum().item()} != {len(batch.batch)}"

                                gen_batch_output.batch['final_mask'] = final_mask
                                gen_batch_output.batch['vanilla_qa_mask'] = vanilla_qa_mask
                                gen_batch_output.batch['subsampled_qa_mask'] = subsampled_qa_mask
                                gen_batch_output.batch['gap_fill_mask'] = gap_fill_mask
                                gen_batch_output.check_consistency()
                            else:
                                if 'vl' in self.config.trainer.experiment_name.lower():
                                    assert 'vl' in self.config.actor_rollout_ref.model.path.lower(), "VL models are required for VL experiments"
                                    gen_batch_output, final_mask, sample_index = self.generation_manager.run_llm_loop_vl(gen_batch, timing_raw)
                                else:
                                ...
                                assert final_mask.sum().item() == len(batch.batch), \
                                    ...
                                    f"{final_mask.sum().item()} != {len(batch.batch)}"
                    ...
                    with marked_timer("reward", timing_raw, color="yellow"):
                        ...
                        elif enable_triple_turn:
                            from recurrent.utils import final_batch
                            # Triple Turn Setting
                            vanilla_qa_reward_batch = final_batch(batch, vanilla_qa_mask, sample_index).union(original_batch)
                            subsampled_qa_reward_batch = final_batch(batch, subsampled_qa_mask, sample_index).union(original_batch)
                            gap_fill_reward_batch = final_batch(batch, gap_fill_mask, sample_index).union(original_batch)
                            gap_fill_reward_batch.non_tensor_batch['reward_model'] = np.array([{'ground_truth': [ans]} for ans in all_gap_fill_answers], dtype=object)
                            vanilla_qa_reward_tensor, vanilla_qa_reward_extra_infos_dict = compute_reward(vanilla_qa_reward_batch, self.reward_fn)
                            subsampled_qa_reward_tensor, subsampled_qa_reward_extra_infos_dict = compute_reward(subsampled_qa_reward_batch, self.reward_fn)
                            gap_fill_reward_tensor, gap_fill_reward_extra_infos_dict = compute_reward(gap_fill_reward_batch, self.reward_fn)
                            subsampled_qa_reward_weight = self.config.algorithm.get("subsampled_qa_reward_weight", 0.5)
                            gap_fill_reward_weight = self.config.algorithm.get("gap_fill_reward_weight", 0.5)
                            print(f'subsampled_qa_reward_weight: {subsampled_qa_reward_weight}, gap_fill_reward_weight: {gap_fill_reward_weight}', flush=True)
                            reward_tensor = (vanilla_qa_reward_tensor + \
                                    subsampled_qa_reward_weight * subsampled_qa_reward_tensor + \
                                    gap_fill_reward_weight * gap_fill_reward_tensor) / (1 + subsampled_qa_reward_weight + gap_fill_reward_weight)
                            reward_extra_infos_dict = vanilla_qa_reward_extra_infos_dict # here we only use vanilla QA reward extra infos
                            # pad for log_prob
                            batch, pad_size = pad_dataproto_to_divisor(batch, self.actor_rollout_wg.world_size)
                        else:
                            # vanilla MemAgent Setting
                            from recurrent.utils import final_batch
                    ...
                    if self.config.recurrent.enable and self.config.algorithm.get("filter_groups", None):  
                        raise NotImplementedError("Filtering is not implemented for recurrent.")
                    ...
                    with marked_timer("adv", timing_raw, color="brown"):
                        ...
                        if not self.config.recurrent.enable:
                            ...
                            if enable_triple_turn:
                                vanilla_qa_advantage_scalar = compute_1D_grpo_advantage(token_level_rewards=vanilla_qa_reward_tensor, 
                                                                                        index=vanilla_qa_reward_batch.non_tensor_batch['uid'],
                                                                                        use_adv=self.config.algorithm.get("norm_adv_by_std_in_grpo", False))
                                vanilla_qa_advantage_scalar = vanilla_qa_advantage_scalar[sample_index]
                                subsampled_qa_advantage_scalar = compute_1D_grpo_advantage(token_level_rewards=subsampled_qa_reward_tensor, 
                                                                                        index=subsampled_qa_reward_batch.non_tensor_batch['uid'],
                                                                                        use_adv=self.config.algorithm.get("norm_adv_by_std_in_grpo", False))
                                subsampled_qa_advantage_scalar = subsampled_qa_advantage_scalar[sample_index]
                                gap_fill_advantage_scalar = compute_1D_grpo_advantage(token_level_rewards=gap_fill_reward_tensor, 
                                                                                        index=gap_fill_reward_batch.non_tensor_batch['uid'],
                                                                                        use_adv=self.config.algorithm.get("norm_adv_by_std_in_grpo", False))
                                gap_fill_advantage_scalar = gap_fill_advantage_scalar[sample_index]
                                advantage_scalar = vanilla_qa_advantage_scalar + \
                                    subsampled_qa_reward_weight * subsampled_qa_advantage_scalar + \
                                    gap_fill_reward_weight * gap_fill_advantage_scalar
                                ...
                                advantages_vanilla_qa = vanilla_qa_advantage_scalar.unsqueeze(-1).tile([1, response_length]) * eos_mask
                                advantages_subsampled_qa = subsampled_qa_advantage_scalar.unsqueeze(-1).tile([1, response_length]) * eos_mask
                                advantages_gap_fill = gap_fill_advantage_scalar.unsqueeze(-1).tile([1, response_length]) * eos_mask

                                # here we combine the advantages of different turns into one advantage tensor:
                                # for intermediate memory-updating turns (~final_mask), the adv should be the aggregation of triple anwering turns
                                # for each final turn, the adv should be the advantage of the corresponding qa_mask turn
                                working_advantages = deepcopy(advantages)
                                working_advantages[~final_mask] = working_advantages[~final_mask]/(1 + subsampled_qa_reward_weight + gap_fill_reward_weight)
                                working_advantages[vanilla_qa_mask] = advantages_vanilla_qa[vanilla_qa_mask]
                                working_advantages[subsampled_qa_mask] = advantages_subsampled_qa[subsampled_qa_mask] * subsampled_qa_reward_weight
                                working_advantages[gap_fill_mask] = advantages_gap_fill[gap_fill_mask] * gap_fill_reward_weight

                                batch.batch['advantages'] = working_advantages
                                batch.batch['advantages_vanilla_qa'] = advantages_vanilla_qa
                                batch.batch['advantages_subsampled_qa'] = advantages_subsampled_qa
                                batch.batch['advantages_gap_fill'] = advantages_gap_fill
                                ...
                                batch.batch['token_level_scores_vanilla_qa'] = vanilla_qa_reward_tensor[sample_index]
                                batch.batch['token_level_scores_subsampled_qa'] = subsampled_qa_reward_tensor[sample_index]
                                batch.batch['token_level_scores_gap_fill'] = gap_fill_reward_tensor[sample_index]
                            else:
                            ...
                            batch.batch['token_level_rewards'] = batch.batch['token_level_scores']
                            print(f"Advantage Computation Done. Start updating actor.", flush=True)
                ...
                print(f"Step {self.global_steps} duration: {steps_duration} seconds", flush=True)
